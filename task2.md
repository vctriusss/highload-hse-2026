...
На главной пользователь получает JSON с данными о его прошлых поездках. Заложим 10 kb на все данные. Тогда с учетом 10k RPS получаем 100Mb/s или 0.8 Gb/s.

При показе геолокации точка кодируется 100 Байт (0.1 Kb). Тогда с учетом 10k RPS итоговая сети требуется 0.008 Gb/s

При создании заказа пользователь передает JSON со всеми параметрами заказа, около 10 Kb. Тогда с учетом 10k RPS получаем 0.8 Gb/s.
...
|Диск (данные) | 17 Gb |
|Сеть (главная)| 0.8 ГБит/с|
|Сеть (геолокация)| 0.8 ГБит/с|
|RPS главной | 10k |
|RPS геолокация| 10k |
